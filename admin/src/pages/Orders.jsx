...
const Orders = () => {
  ...
  const handleFilter = () => {
    const filtered = orders.filter((order) => {
      const matchesSearch = search
        ? getUserName(order.user).toLowerCase().includes(search.toLowerCase())
        : true;
      const matchesStatus = statusFilter ? order.status === statusFilter : true;
      const matchesMethod = methodFilter ? order.paymentMethod === methodFilter : true;
  ...
  const handleExportExcel = async () => {
    ...
    worksheet.columns = [
      { header: "Invoice No", key: "_id", width: 25 },
    ...
    link.download = "Admin_Orders.xlsx";
    link.click();
  };

  const handlePrint = async (order) => {
    const logoUrl = "../logo.png";
    const { _id, items = [], paymentMethod, createdAt, shippingAddress, totalAmount = 0 } = order;
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.text(getUserName(order.user), 40, 185);
    ...
    doc.text(`GRAND TOTAL: ${formatPrice(totalAmount)}`, 470, totalY + 22, { align: "center" });
    ...
    <div className="px-10 py-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          ...
          <p className="mt-1" style={{ color: "#666666" }}>
            Manage and track all store orders efficiently.
          </p>
        </div>
        <button
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow-md border border-gray-100">
        <table className="min-w-full table-auto">
          <thead className="bg-[#ff6600]/10 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              <th className="py-3 px-5 text-left">Invoice No</th>
          ...
          <tbody className="text-sm">
            {filteredOrders.length > 0 ? (
              filteredOrders.map((order) => (
                <tr
                  key={order._id}
                  className="border-b hover:bg-[#ff6600]/5 transition-colors duration-150"
                >
                  <td
                  ...
                  >
                    <div className="flex flex-col justify-center items-center h-full gap-1">
                      <span className="text-[0.75rem]">
                        #{order._id.slice(0, order._id.length / 2).toUpperCase()}
                      </span>
                      <span className="text-[0.75rem]">
                        {order._id.slice(order._id.length / 2).toUpperCase()}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-5 text-gray-600">{getUserName(order.user)}</td>
                  <td className="py-3 px-5 text-gray-600">{order.paymentMethod}</td>
                  <td className="py-3 px-5 text-gray-600 text-left whitespace-nowrap">
      ...
      {showModal && selectedOrder && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="relative bg-white rounded-xl p-6 w-[400px] mx-auto shadow-xl z-50">
            <button
            ...
            <div className="text-sm text-gray-600 space-y-2">
              <p>
                <strong>Invoice:</strong> #{selectedOrder._id.slice(0, selectedOrder._id.length / 2).toUpperCase()}
                {selectedOrder._id.slice(selectedOrder._id.length / 2).toUpperCase()}
              </p>
              <p>
                <strong>Customer:</strong> {getUserName(selectedOrder.user)}
              </p>
              <p>
              ...
              <p>
                <strong>Amount:</strong> {formatPrice(selectedOrder.totalAmount)}
              </p>
              <p>
