...
const res = await axios.get(`${API_URL}/api/products`);
  ...
  const handleToggleNew = async (id, currentStatus) => {
    ...
    try {
      ...
      toast.success(`Product ${!currentStatus ? "marked as new" : "removed from new"}`);
    } catch (err) {
      console.error("❌ Error updating new status:", err);
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 sm:px-6 py-10">
      ...
      <div className="flex flex-col items-start gap-1 mb-7 animate-fadeIn">
        <h1 className="text-3xl font-bold text-[#e65a00]">Manage New Collections</h1>
        <p className="text-[#666666]">
          Enable, edit, or delete new products below.
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200 animate-slideUp">
        ...
        {loading ? (
          ...
          <p className="text-center py-16 text-gray-500 text-lg">No products found.</p>
        ) : (
            ...
            <table className="min-w-full border-t text-sm sm:text-base">
              <thead className="bg-[#fff5f0] text-gray-700">
                <tr>
                  ...
                  <th className="px-5 py-3 text-center font-semibold">New</th>
                ...
                {products.map((p) => (
                  <tr key={p._id} className="hover:bg-orange-50 transition-all">
                    ...
                    <td className="px-5 py-3 text-center">
                      <Switch
                        checked={p.isNew}
                        onChange={() => handleToggleNew(p._id, p.isNew)}
                        className={`${p.isNew ? "bg-[#ff6600]" : "bg-gray-300"
                          } relative inline-flex h-6 w-11 items-center rounded-full transition-all`}
                      >
                        <span
                          className={`${p.isNew ? "translate-x-6" : "translate-x-1"
      ...
      {deleteModal.open && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-lg p-6 w-[90%] max-w-sm animate-slideUp">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Delete Product?</h3>
